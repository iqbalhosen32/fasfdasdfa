...
function Master({ children, ns, title, subTitle, first }) {
  console.log(first)
  ...
  return (
    <div
      ...
      <div
        className={`grid gap-6 overflow-hidden xs:gap-0 sm:gap-0 xl:grid-rows-[46px,1fr] 2xl:grid-rows-[46px,1fr] 3xl:grid-rows-[46px,1fr]  ${ns
        ...
        <div
          className={`${router.pathname === "/ruqyah/video" || router.pathname === "/ruqyah/video/details"
            ? "hidden"
      ...
      {/* <MemorizeDetails dua={dua} /> */}
      {
        first?.map((item) => <MemorizeDetails dua={item} />)
      }
    </div>
  );
}
